...
def train_collie(
    ...
    # EasyProject Training data
    if data_args.train_ep_tasks:
        logging.info(
            f"We will train CoLLIE on {len(training_ep_datasets_path)} datasets: {', '.join(training_ep_datasets_path)}"
        )
        ...
        for train_task in data_args.train_ep_tasks:
            ...
            if os.path.exists(train_path):
                train_dataset = CollieDataset(
                    ...
                    inference=False,
                    prompt_loss_weight=data_args.prompt_loss_weight,
                    max_examples=data_args.max_examples_per_task_train_ep,
                )
                training_datasets.append(train_dataset)
    ...
    if data_args.train_tf_tasks:
        logging.info(
            f"We will train CoLLIE on {len(training_tf_datasets_path)} datasets: {', '.join(training_tf_datasets_path)}"
        )
        ...
        for train_task in data_args.train_tf_tasks:
            ...
            if os.path.exists(train_path):
                train_dataset = CollieDataset(
                    ...
                    inference=False,
                    prompt_loss_weight=data_args.prompt_loss_weight,
                    max_examples=data_args.max_examples_per_task_train_tf,
                )
                training_datasets.append(train_dataset)
