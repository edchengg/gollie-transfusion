...
import json
from src.tasks.multito.guidelines import GUIDELINES
from src.tasks.multito.guidelines_gold import EXAMPLES
from src.tasks.multito.prompts import (
    ENTITY_DEFINITIONS,
    WeatherNoun,
    Location, 
    Datetime, 
...
class MultiTODatasetLoader(DatasetLoader):
    """
...
class MultiTOTransFusionDatasetLoader(DatasetLoader):
    """
    A `DatasetLoader` for the ConLL03 dataset.
    ...
    def __init__(self, path_or_split: str, include_misc: bool = True, **kwargs) -> None:
        ...
        lang = path_or_split.split("/")[-1].split(".")[0].split("-")[-1]
        translation = load_jsonl(f"data_translatetest/multito/{lang}.eng_Latn.jsonl")
...
class MultiTOSampler(Sampler):
    ...
    def __init__(
        self,
        dataset_loader: MultiTODatasetLoader,
        task: str = None,
        split: str = "train",
