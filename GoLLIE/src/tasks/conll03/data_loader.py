...
def load_jsonl(path, ENTITY_TO_CLASS_MAPPING):
    ...
    # load data - remove MISC
    for out in output:
        tgt_words = out["tgt_words"]
        src_words = out["src_words"]
        tgt_labels = out["tgt_labels"]
        src_labels = out["src_labels"]

        tgt_entities = get_entities(tgt_labels, tgt_words, ENTITY_TO_CLASS_MAPPING)
        src_entities = get_entities(src_labels, src_words, ENTITY_TO_CLASS_MAPPING)

        dataset_sentences.append(tgt_words)
        dataset_entities.append(tgt_entities)
    ...
    def __init__(self, path_or_split: str, include_misc: bool = True, **kwargs) -> None:
        ...
        for id, (words, entities, words_en, entities_en) in enumerate(zip(dataset_words, dataset_entities, dataset_words_en, dataset_entities_en)):
            ...
            self.elements[id] = {
                ...
                "doc_id": id,
                "text": " ".join(words),
                "en_text": " ".join(words_en),
                "entities": entities,
                "en_entities": entities_en,
                "gold": entities,
                "en_gold": entities_en,
            }
